    ...
    return (
        <div className="border-2 p-6 rounded-3xl border-accent-2 bg-light/30 backdrop-blur-md">
            <div className="flex gap-6 flex-wrap">
                <Image 
                    className="min-w-96 min-h-44 rounded-3xl max-w-[504px] max-h-[288px] w-full h-full" 
                    width={384} 
                    height={176} 
                    src={data.image} 
                    alt={data.title[lang]}
                ...
                <div>
                    ...
                    <p className="text-white text-2xl font-bold mt-6">{data.description[lang]}</p>
                </div>
            </div>
            <div className="flex mt-6 gap-6 flex-wrap">
                <div className="min-w-96 max-w-[504px] w-full">
                ...
                <div className="flex justify-between flex-wrap gap-6">
                    {data.tags.map((tag: string, key: number) => 
                        <div className="py-2 px-6 flex-1 h-12 bg-accent-1 text-center text-white rounded-full text-2xl font-bold" key={key}>{tag}</div>
                    )}
                </div>
            </div>
        </div>
